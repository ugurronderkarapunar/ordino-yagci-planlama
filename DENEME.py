"""
Ordino Yağcı Planlaması — Tüm Butonlara help Eklendi (v5.8)
...
def saat_cakisiyor(bas1: str, bit1: str, bas2: str, bit2: str) -> bool:
    def to_minutes(saat: str) -> int:
        ...
        return h * 60 + m
    b1 = to_minutes(bas1); e1 = to_minutes(bit1)
    b2 = to_minutes(bas2); e2 = to_minutes(bit2)
    if e1 <= b1: e1 += 24 * 60
    if e2 <= b2: e2 += 24 * 60
...
def onerileri_hesapla(gemi_id, makine_tipi_id, hedef_tarih, cikan_id=None, limit=5, esnek_cakisma=False):
    ...
    for p in tum:
        ...
        toplam_puan = vardiya_puan + kalite_puan + mesai_ceza + dinlenme_cezasi + pespese_cezasi + nlp_etki + ust_uste_ceza
        if vardiya == "IZINCI":
            toplam_puan += 200
        sonuclar.append({**p, "puan":toplam_puan, "uyari_8_5":vardiya=="8_5", "zaten_atanmis":False, "fazla_mesai":fazla, "son_7_gun":gun, "dinlenme_ihlali":dinlenme_cezasi<0, "pespese":pespese_cezasi<0, "nlp_puan":nlp_puan, "bas_saat":bas_saat, "bit_saat":bit_saat, "ust_uste":ust_uste_ceza<0})
...
def to_dict_rows(oneriler):
    ...
    for o in oneriler:
        ...
        if o.get("pespese"): ad += " 🔁 AYNI GEMİ PEŞ PEŞE"
        if o.get("ust_uste"): ad += " 🔄 DÜN ÇALIŞTI"
        if o.get("nlp_puan", 0) < -0.3: ad += " 📝 OLUMSUZ YORUM"
        elif o.get("nlp_puan", 0) > 0.3: ad += " 👍 OLUMLU YORUM"
        rows.append({"id":o["id"], "ad_soyad":ad, "vardiya":o.get("vardiya_tipi","-"), "makine":makine_str, "puan":o["puan"], "uyari_8_5":o.get("uyari_8_5",False), "zaten_atanmis":o.get("zaten_atanmis",False)})
    return rows
...
def pdf_rapor_olustur(tip="aylik_ozet", ay=None, yil=None, baslangic=None, bitis=None):
    ...
    if tip == "aylik_ozet":
        ...
        if baslangic and bitis:
            rows = sql_all("SELECT v.tarih,g.ad AS gemi,m.ad AS makine,p.ad||' '||p.soyad AS personel FROM vardiya_plan v JOIN gemi g ON v.gemi_id=g.id JOIN makine_tipi m ON v.makine_tipi_id=m.id JOIN personel p ON v.personel_id=p.id WHERE v.tarih BETWEEN ? AND ? ORDER BY v.tarih DESC",
                           (baslangic.isoformat(), bitis.isoformat()))
        else:
            rows = sql_all("SELECT v.tarih,g.ad AS gemi,m.ad AS makine,p.ad||' '||p.soyad AS personel FROM vardiya_plan v JOIN gemi g ON v.gemi_id=g.id JOIN makine_tipi m ON v.makine_tipi_id=m.id JOIN personel p ON v.personel_id=p.id ORDER BY v.tarih DESC LIMIT 50")
        for p in rows:
...
def test_verisi_olustur():
    ...
    sql_run("INSERT INTO izin(personel_id,baslangic,bitis,gun_sayisi,notlar) VALUES(?,?,?,?,?)",
            (p_map["Fatma AYDIN"], (bugun+timedelta(days=5)).isoformat(), (bugun+timedelta(days=7)).isoformat(), 3, "planlı izin"))
    sql_run("INSERT INTO izin(personel_id,baslangic,bitis,gun_sayisi,notlar) VALUES(?,?,?,?,?)",
            (p_map["Hasan OZTURK"], (bugun+timedelta(days=1)).isoformat(), (bugun+timedelta(days=2)).isoformat(), 2, "2 gün mazeret"))
    sql_run("INSERT INTO izin(personel_id,baslangic,bitis,gun_sayisi,notlar) VALUES(?,?,?,?,?)",
            (p_map["Huseyin ARSLAN"], bugun.isoformat(), bugun.isoformat(), 1, "1 günlük izin"))
    sql_run("INSERT INTO izin(personel_id,baslangic,bitis,gun_sayisi,notlar) VALUES(?,?,?,?,?)",
            (p_map["YEDEK2 KISI"], (bugun-timedelta(days=1)).isoformat(), (bugun+timedelta(days=1)).isoformat(), 3, "yedek izinde"))
    ...
    st.success("Kapsamlı test verileri oluşturuldu (gemi-makine eşleştirmeli)!")
    st.rerun()

# ---------- YENİ YARDIMCILAR ----------
def son_gorev_gun_farki(pid):
    row = sql_one("SELECT MAX(tarih) AS son FROM vardiya_plan WHERE personel_id=?", (pid,))
    if row and row["son"]:
        son_tarih = date.fromisoformat(row["son"])
        return (date.today() - son_tarih).days
    return None

def sertifika_uyarilari_al():
    bugun = date.today()
    yaklasan = sql_all("SELECT p.ad, p.soyad, m.ad AS makine, s.sertifika_adi, s.gecerlilik_tarihi "
...
def bugun_plani_olustur():
    ...
    makine_tipleri = sql_all("SELECT id,ad FROM makine_tipi ORDER BY ad")
    plan = []
    for gemi in gemiler:
        gemi_mak = sql_all("SELECT makine_tipi_id FROM gemi_makine WHERE gemi_id=?", (gemi["id"],))
        for gm in gemi_mak:
            mak = next((m for m in makine_tipleri if m["id"]==gm["makine_tipi_id"]), None)
            if not mak: continue
            row = sql_one("SELECT p.ad||' '||p.soyad AS isim FROM vardiya_plan v JOIN personel p ON v.personel_id=p.id WHERE v.gemi_id=? AND v.makine_tipi_id=? AND v.tarih=?", (gemi["id"], mak["id"], bugun))
            plan.append({
                "Gemi": gemi["ad"],
                "Makine": mak["ad"],
...
def _sayfa_yapboz():
    ...
    with c_btns:
        ...
        if cols_hafta[0].button("⬅️ Hafta", help="Bir önceki haftaya gider"):
            sec_tarih = st.session_state.yapboz_tarih - timedelta(days=7)
            st.session_state.yapboz_tarih = sec_tarih
            st.rerun()
        if cols_hafta[1].button("Hafta ➡️", help="Bir sonraki haftaya gider"):
            sec_tarih = st.session_state.yapboz_tarih + timedelta(days=7)
            st.session_state.yapboz_tarih = sec_tarih
            st.rerun()
    ...
    if not gemiler or not tum_makineler: st.warning("Gemi ve makine ekleyin."); return
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🧹 Tüm Atamaları Temizle", key="temizle_yapboz", type="secondary", help="Seçili tarihteki tüm vardiyaları siler"):
            sql_run("DELETE FROM vardiya_plan WHERE tarih=?", (sec_tarih.isoformat(),))
            st.toast("Tüm atamalar temizlendi!", icon="🧹")
            st.rerun()
    with col2:
        if st.button("🤖 Hepsini Otomatik Doldur", key="otomatik_doldur_btn", help="Sistemin önerdiği en uygun personelle boşlukları doldurur"):
        ...
        for i, mak in enumerate(g_makineler):
            with cols[i]:
                ...
                if mevcut:
                    ...
                    with col_x:
                        if st.button("❌ Çıkar", key=f"c_{gemi['id']}_{mak['id']}_{sec_tarih}", help="Bu personeli vardiyadan çıkarır"):
                            ...
                            st.rerun()
                    with col_d:
                        if st.button("🔄 Değiştir", key=f"degistir_{gemi['id']}_{mak['id']}_{sec_tarih}", help="Çalışanı çıkarıp hemen yeni öneri getirir"):
                            sql_run("DELETE FROM vardiya_plan WHERE gemi_id=? AND makine_tipi_id=? AND tarih=?",(gemi["id"],mak["id"],sec_tarih.isoformat()))
                    ...
                    for p in sql_all("SELECT * FROM personel WHERE aktif=1 AND (vardiya_tipi='IZINCI' OR durum IN ('Gemide','İskelede'))"):
                        if p["id"] in izinli:
                            engel_izinli += 1
                            continue
                        if baska_gemide_mi(p["id"], sec_tarih, gemi["id"]):
                            engel_baska_gemi += 1
                            continue
                        if ayni_gun_baska_makine(p["id"], sec_tarih, mak["id"]):
                            continue
                        if p["vardiya_tipi"] != "IZINCI":
                            ...
                            if gunler_json:
                                try:
                                    izin_gunler = json.loads(gunler_json)
                                    if isinstance(izin_gunler, list) and izin_gunler:
                                        if hedef_gun not in izin_gunler:
                                            engel_gun += 1
                                            continue
                                except: pass
                        mids = _id_listesi(p.get("makine_tipi_id_list"))
                        if mids and mak["id"] not in mids:
                            engel_makine += 1
                            continue
                        if mids and not sertifika_gecerli_mi(p["id"], mak["id"], sec_tarih):
                            engel_sertifika += 1
                            continue
                        gids = _id_listesi(p.get("gemi_id_list"))
                    ...
                    if len(uygun)==1:
                        st.caption(f"Uygun yok. İzinli: {engel_izinli}, Başka gemide: {engel_baska_gemi}, "
                                   f"Uygun gün değil: {engel_gun}, Makine yetkinliği yok: {engel_makine}, "
                                   f"Sertifika eksik: {engel_sertifika}")
                    else:
                            ...
                            if pid_row:
                                ...
                                if saat_cakismasi_var(pid_row["id"], sec_tarih, bas_saat, bit_saat):
                                    ...
                                    if iki_gun_ust_uste_mi(pid_row["id"], sec_tarih):
                                        st.warning("⚠️ Bu personel dün de çalıştı. Arka arkaya iki gün çalışıyor!")
                                    try:
                                        sql_run("INSERT INTO vardiya_plan(personel_id,gemi_id,makine_tipi_id,tarih,baslangic_saat,bitis_saat) VALUES(?,?,?,?,?,?)",
                    ...
                    if st.button("🔍 Öneri Al (5)", key=f"onerbtn_{gemi['id']}_{mak['id']}_{sec_tarih}", help="En uygun 5 personeli listeler"):
                    ...
                    if f"oneriler_{gemi['id']}_{mak['id']}" in st.session_state and st.session_state[f"oneriler_{gemi['id']}_{mak['id']}"]:
                        ...
                        for o in oneriler:
                            ...
                            with col_o1:
                                ...
                                st.write(puan_str)
                            with col_o2:
                                if st.button("✅ Ata", key=f"ata_{gemi['id']}_{mak['id']}_{o['id']}", help="Bu kişiyi seçili pozisyona atar"):
    ...
    with st.expander("🔗 Gemi-Makine Eşleştirme (Düzenle)"):
        ...
        if sec_gemi:
            ...
            sec_mak = st.multiselect("Gemideki Makineler", [m["id"] for m in makineler],
                                     default=mevcut_mak, format_func=lambda i: next((m["ad"] for m in makineler if m["id"]==i), ""), key="gm_mak")
            if st.button("Güncelle", key="gm_guncelle", help="Seçili makineleri gemiyle eşleştirir"):
    ...
    with c1:
        with st.expander("✏️ Gemi Düzenle/Sil"):
            if g_rows:
                ...
                if st.button("Güncelle",key="bgd", help="Geminin adını, kodunu veya konumunu günceller"):
                ...
                if st.button("Sil",key="bgs",type="secondary", help="Gemiyi tüm bağlantılarıyla siler"):
                    personel_var = sql_one("SELECT COUNT(*) AS c FROM personel WHERE gemi_id=?",(gr["id"],))["c"]
                    makine_var = sql_one("SELECT COUNT(*) AS c FROM gemi_makine WHERE gemi_id=?",(gr["id"],))["c"]
                    if personel_var > 0:
                        st.error("Bağlı personel var, önce onları başka gemiye atayın veya silin.")
                    else:
                        if makine_var > 0:
                            st.warning(f"Bu gemide {makine_var} makine tanımı silinecek.")
    ...
    with c2:
        with st.expander("✏️ Makine Düzenle/Sil"):
            ...
            if mr:
                ...
                if st.button("Güncelle",key="bmd", help="Makine adını günceller"):
                ...
                if st.button("Sil",key="bms",type="secondary", help="Makineyi siler (bağlı personel varsa izin vermez)"):
                    if mrow["c"]>0: st.error("Bağlı personel var")
                    else: sql_run("DELETE FROM gemi_makine WHERE makine_tipi_id=?",(mrow["id"],)); sql_run("DELETE FROM vardiya_plan WHERE makine_tipi_id=?",(mrow["id"],)); sql_run("DELETE FROM makine_tipi WHERE id=?",(mrow["id"],)); st.toast("Makine silindi!", icon="🗑️"); st.rerun()
...
def _sayfa_personel():
    ...
    with st.expander("🔄 Toplu Durum Değiştir"):
        ...
        if st.button("Durumları Güncelle", key="toplu_durum_btn", help="Seçili personelin durumunu topluca günceller"):
            if sec_personel:
    ...
    if kart_sec:
        ...
        if p:
            with st.expander(f"📋 {p['ad']} {p['soyad']} - Detaylı Kart", expanded=True):
                ...
                mids = _id_listesi(p.get("makine_tipi_id_list"))
                tum_mak = {r["id"]: r["ad"] for r in sql_all("SELECT id,ad FROM makine_tipi")}
                mak_adlari = ", ".join(tum_mak.get(m, "?") for m in mids) if mids else "Tüm Makineler"
                with col1:
                ...
                with col2:
                    ...
                    bos_gun = son_gorev_gun_farki(pid)
                    if bos_gun is not None:
                        st.markdown(f"**Son Görev:** {bos_gun} gün önce")
                    else:
                        st.markdown("**Son Görev:** Hiç yok")
                bugun = date.today()
                son_7_gun = [(bugun - timedelta(days=i)).isoformat() for i in range(7)]
                calisma_gunleri = sql_all("SELECT v.tarih, g.ad AS gemi, m.ad AS makine FROM vardiya_plan v JOIN gemi g ON v.gemi_id=g.id JOIN makine_tipi m ON v.makine_tipi_id=m.id WHERE v.personel_id=? AND v.tarih >= ?", (pid, (bugun - timedelta(days=7)).isoformat()))
                ...
                for gun in son_7_gun:
                    calisan = [g for g in calisma_gunleri if g["tarih"] == gun]
                    if calisan:
                        gunler_str += f"🟢 {gun} ({calisan[0]['gemi']} - {calisan[0]['makine']}) | "
    ...
    with st.expander("📤 Excel'den Toplu Personel Ekle"):
        st.markdown("Sütunlar: **Ad, Soyad, Vardiya Tipi, Makine Tipi (virgülle ayrılabilir), Gemi, Durum, Performans Notu**")
        ...
        if personel_excel is not None:
            try:
                ...
                if st.button("📤 Ekle", key="btn_toplu_personel", help="Excel'deki tüm satırları personel olarak kaydeder"):
                    eklenen = 0
                    for _, row in df.iterrows():
                        ...
                        if gemi_adi_str:
                            ...
                            if not gemi:
                                try:
                                    sql_run("INSERT INTO gemi(ad) VALUES(?)", (gemi_adi_str,))
                                    gemi = sql_one("SELECT id FROM gemi WHERE ad=?", (gemi_adi_str,))
                                except: continue
    ...
    with st.expander("➕ Yeni Personel"):
        ...
        mak_sec=st.multiselect("Makine Tipleri (boş = tüm makineler)",[r["id"] for r in makineler],format_func=lambda i:next(r["ad"] for r in makineler if r["id"]==i),key="p_mak")
        gem_list=st.multiselect("Atandığı Gemiler (boş = tüm gemiler)",[r["id"] for r in gemiler],format_func=lambda i:next(r["ad"] for r in gemiler if r["id"]==i),key="p_gem")
        gem_tek=int(gem_list[0]) if gem_list else None
        sec=st.multiselect("Vardiya Günleri (İZİNCİ için fark etmez)",GUNLER_TR,default=["Pazartesi","Çarşamba","Cuma"],key="p_vg")
        ...
        pn=st.text_area("Performans Notu (NLP analizine tabidir)",key="p_not")
        if st.button("Kaydet",key="btn_pk", help="Yeni personeli sisteme ekler"):
                ...
                try:
                    sql_run("INSERT INTO personel(ad,soyad,gemi_id,gemi_id_list,makine_tipi_id,makine_tipi_id_list,vardiya_tipi,vardiya_gunleri,gemi_tutumu,izin_saat_araligi,is_kalitesi,performans_notu,durum) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            (ad.strip().upper(),soyad.strip().upper(),
                             gem_tek, _gemi_id_json(gem_list),
                             int(mak_sec[0]) if mak_sec else None,
                             _makine_id_json(mak_sec) if mak_sec else "[]",
                             vt, gun_json,
                             gemi_tut,
                             f"{ib.strftime('%H:%M')} - {it.strftime('%H:%M')}" if ib and it else None,
                             is_kal, pn.strip() or None, durum))
    ...
    with st.expander("✏️ Düzenle/Sil & Sertifika"):
        ...
        ymak=st.multiselect("Makine Tipleri (boş = tüm makineler)",[r["id"] for r in makineler],default=[m for m in mids if m in [r["id"] for r in makineler]],format_func=lambda i:next(r["ad"] for r in makineler if r["id"]==i),key="pd_mak")
        gids=_id_listesi(mev.get("gemi_id_list")) or ([mev["gemi_id"]] if mev.get("gemi_id") else [])
        ygem=st.multiselect("Atandığı Gemiler (boş = tüm gemiler)",[r["id"] for r in gemiler],default=[g for g in gids if g in [r["id"] for r in gemiler]],format_func=lambda i:next(r["ad"] for r in gemiler if r["id"]==i),key="pd_gem")
        ...
        ypn = st.text_area("Performans Notu (NLP)", value=mev.get("performans_notu") or "", key="pd_pn")
        c1,c2=st.columns(2)
        if c1.button("Güncelle",key="bpgu", help="Personel bilgilerini günceller"):
            try:
        ...
        if c2.button("Sil",key="bps",type="secondary", help="Personeli sistemden tamamen siler"):
        ...
        with st.form("sert_ekle",clear_on_submit=True):
            ...
            if st.form_submit_button("Ekle", help="Yeni sertifika ekler"):
                sql_run("INSERT INTO personel_sertifika VALUES(NULL,?,?,?,?,?)",(pid,sm,sa or None,sg.isoformat() if sg else None,sn or None)); st.toast("Sertifika eklendi!", icon="🎓"); st.rerun()
        if sert:
            sil_s=st.selectbox("Silinecek",[f"{s['sertifika_adi'] or 'Sertifika'} (ID:{s['id']})" for s in sert],key="sils")
            if st.button("Sertifika Sil",key="bss", help="Seçili sertifikayı siler"):
                sid=int(sil_s.split("ID:")[1].replace(")","")); sql_run("DELETE FROM personel_sertifika WHERE id=?",(sid,)); st.toast("Sertifika silindi!", icon="🗑️"); st.rerun()
...
def _sayfa_izin():
    ...
    with st.expander("📤 Excel'den Toplu İzin Ekle"):
        ...
        if izin_excel is not None:
            try:
                ...
                st.dataframe(df.head(), use_container_width=True)
                if st.button("📤 Ekle", key="btn_toplu_izin", help="Excel'deki izinleri topluca sisteme aktarır"):
                    eklenen = 0
                    for _, row in df.iterrows():
                        ad = str(row.get('Ad', '')).strip().upper()
                        soyad = str(row.get('Soyad', '')).strip().upper()
    ...
    with cf:
        ...
        pid=sec["id"]; bas=st.date_input("Başlangıç",value=date.today(),key="izb",format="DD.MM.YYYY"); bit=st.date_input("Bitiş",value=date.today(),key="izbi",format="DD.MM.YYYY")
        ...
        if st.button("✅ Kaydet",key="biz", help="İzni kaydeder. Görev çakışırsa uyarı gösterir"):
            if gun<=0: st.error("Geçersiz aralık")
            else:
                calisan_gorevler = sql_all("SELECT DISTINCT v.tarih, g.ad || ' - ' || m.ad AS pozisyon "
                                           "FROM vardiya_plan v JOIN gemi g ON v.gemi_id=g.id "
                                           "JOIN makine_tipi m ON v.makine_tipi_id=m.id "
                                           "WHERE v.personel_id=? AND v.tarih BETWEEN ? AND ?",
                                           (pid, bas.isoformat(), bit.isoformat()))
                if calisan_gorevler:
                    st.warning("⚠️ Bu personelin aşağıdaki görevleri çakışıyor. Yine de izin verilsin mi?")
                    for gv in calisan_gorevler:
                        st.write(f"- {gv['tarih']} → {gv['pozisyon']}")
                sql_run("INSERT INTO izin VALUES(NULL,?,?,?,?,?,?)",(pid,bas.isoformat(),bit.isoformat(),gun,notlar or None,None))
                st.toast("İzin kaydedildi!", icon="✅")
                st.rerun()
    with cc:
        ...
        for iz in sql_all("SELECT baslangic,bitis FROM izin WHERE personel_id=?",(pid,)):
            ...
            while d<=b:
                ...
                d+=timedelta(days=1)
        st.markdown(_takvim_html(yil,ay_i,isaret,koyu), unsafe_allow_html=True)
    ...
    if not izinler: st.info("İzin yok")
        ...
        for iz in izinler:
            ...
            if c3.button("🗑️",key=f"izsil_{iz['id']}", help="Bu izin kaydını siler"):
                sql_run("DELETE FROM izin WHERE id=?",(iz["id"],)); st.toast("İzin silindi!", icon="🗑️"); st.rerun()
...
def _sayfa_carkci():
    ...
    c1,c2=st.columns(2)
    with c1: ad=c1.text_input("Ad",key="cka"); soyad=c2.text_input("Soyad",key="cks"); gid=c1.selectbox("Gemi",[r["id"] for r in gem],format_func=lambda i:next(r["ad"] for r in gem if r["id"]==i),key="ckg"); cvt=c1.selectbox("Vardiya",VARDIYA_TIPLERI,key="ckv"); cg=c1.multiselect("Günler",GUNLER_TR,key="ckgun")
    with c2: yop=[("(Seçilmedi)",None)]+[(f"{p['ad']} {p['soyad']}",p["id"]) for p in yag]; ys=c2.selectbox("Sorunlu Yağcı",yop,format_func=lambda x:x[0],key="cky"); sorun=c2.text_area("Sorun / Açıklama (NLP)",key="ckso"); vn=c2.text_input("Vardiya Notu",key="ckvn"); pk=c2.slider("Puan Kırma",0,5,0,key="ckp")
    if st.button("Oluştur",key="bck", help="Çarkçı kaydını kaydeder ve varsa personel puanını düşürür"):
        if not ad or not soyad: st.error("Ad soyad zorunlu")
        else:
...
def _sayfa_acil():
    ...
    with st.expander("📅 Dün Kim Çalıştı?"):
        ...
        if st.button("🔍 Sorgula", key="dun_sorgu", help="Dünkü pozisyonun sahibini gösterir"):
    ...
    with st.expander("🏝️ Personeli İskeleye Çıkar"):
        ...
        if st.button("🔄 İskeleye Çıkar", key="btn_iskele", help="Personeli 'İskelede' yapar"):
    ...
    with st.expander("🚀 İskeledekileri Akıllı Dağıt"):
        if st.button("🧠 Akıllı Dağıtım Başlat", key="btn_akilli_dagit", help="İskelede/İZİNCİ personeli boş yerlere otomatik yerleştirir"):
    ...
    if st.button("🔍 Listele",key="bbos", help="Bugün hiç görevi olmayan personeli listeler"):
        bos=[]
        for p in sql_all("SELECT * FROM personel WHERE aktif=1 AND (vardiya_tipi='IZINCI' OR durum IN ('Gemide','İskelede'))"):
    ...
    if st.button("🔍 İskele Listesi",key="biskele", help="'İskelede' durumundaki personeli gösterir"):
    ...
    if st.button("🔍 Tersane Listesi",key="btersane", help="Tersanedeki gemilere uygun personeli listeler"):
        ...
        if not ters_gem: st.warning("Tersanede gemi yok")
            ...
            for g in ters_gem:
                for p in sql_all("SELECT * FROM personel WHERE aktif=1 AND (gemi_id=? OR gemi_id_list LIKE ? OR (gemi_id IS NULL AND gemi_id_list='[]'))",(g["id"],f'%{g["id"]}%')):
                    if p["id"] in izinli or izinde_mi(p["id"],bugun): continue
                    mids=_id_listesi(p.get("makine_tipi_id_list"))
    ...
    if st.button("🚨 Öner (5)",key="bacil", help="İzin isteyenin yerine en uygun 5 adayı getirir"):
...
def _sayfa_oneri():
    ...
    st.info("💡 **Öneri Motoru:** İZİNCİ personel gün kısıtlamasız, yüksek önceliklidir. Arka arkaya iki gün çalıştırılırsa uyarı alırsınız.")
    ...
    with st.expander("Ayarlar"):
        ...
        if sg:
            ...
            sm = st.multiselect("Makine (seçili gemilerde olanlar)", [m["id"] for m in mak_secenek],
                                format_func=lambda i: next((m["ad"] for m in mak_secenek if m["id"]==i), ""), key="tpm2")
        else:
            sm = []
        ...
        if st.button("🚀 Oluştur",key="btp", help="Seçili aralıktaki tüm boş pozisyonları adilce doldurur"):
    ...
    with st.expander("🗑️ Vardiya Sil"):
        ...
        if mevcut_sil:
            ...
            if st.button("Atamayı Sil",key="sil_ata", help="Belirtilen atamayı kaldırır"):
                sql_run("DELETE FROM vardiya_plan WHERE gemi_id=? AND makine_tipi_id=? AND tarih=?",(sil_gemi,sil_mak,sil_tarih.isoformat())); st.toast("Atama silindi!", icon="🗑️"); st.rerun()
    ...
    if not mak_secenek:
        st.warning("Bu gemide makine tanımlı değil!")
        return
    ...
    if st.button("🔍 Öner",key="bon", help="Seçili pozisyon için en uygun 5 adayı listeler"):
    ...
    if st.button("✅ Kaydet",key="bvk", help="Seçili personeli elle atar"):
...
def _sayfa_bilgi():
    ...
    with col1:
        st.metric("👥 Toplam Personel", sql_one("SELECT COUNT(*) AS c FROM personel WHERE aktif=1")["c"])
    with col2:
    ...
    with col3:
        ...
        st.metric("🏝️ Bugün İzinde", izinli_sayi)
    st.divider()
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("💾 Yedekle",key="byedek", help="Veritabanının yedeğini alır"):
            st.toast(f"Yedek: {veritabani_yedekle().name}", icon="💾")
    with c2:
        if st.button("🧪 Test Verisi",key="btest", help="Örnek gemi, makine, personel ve izin kayıtları oluşturur"):
            test_verisi_olustur()
    with c3:
        st.download_button("📥 DB İndir",open(DB_PATH,"rb"),file_name=f"ordino_{date.today().isoformat()}.db", help="Veritabanı dosyasını bilgisayara indirir")
    st.divider()
    st.subheader("📄 PDF")
    cp1,cp2=st.columns(2)
    with cp1:
        if st.button("Aylık Özet PDF",key="bpdfa", help="Aylık personel özeti PDF'i oluşturur"):
    ...
    with cp2:
        ...
        if st.button("PDF Oluştur", key="pdf_aralik", help="Seçili aralıktaki vardiya planını PDF yapar"):
            p = pdf_rapor_olustur("vardiya_plani", baslangic=p_bas, bitis=p_bit)
            st.download_button("İndir", open(p,"rb"), file_name=p.name)
    st.divider()
    st.subheader("🕐 Son 24 Saat")
    son_24 = (datetime.now() - timedelta(hours=24)).isoformat()
    son_atamalar = sql_all("SELECT v.tarih, g.ad AS gemi, m.ad AS makine, p.ad||' '||p.soyad AS personel, v.baslangic_saat, v.bitis_saat FROM vardiya_plan v JOIN gemi g ON v.gemi_id=g.id JOIN makine_tipi m ON v.makine_tipi_id=m.id JOIN personel p ON v.personel_id=p.id WHERE v.tarih >= ? ORDER BY v.tarih DESC", (son_24,))
    if son_atamalar: st.dataframe(pd.DataFrame(son_atamalar), use_container_width=True, hide_index=True)
    else: st.info("Son 24 saatte atama yok.")
    st.divider()
    ...
    if bugun_plani:
        ...
        metin = ""
        for row in bugun_plani:
            metin += f"{row['Gemi']} - {row['Makine']}: {row['Personel']}\n"
        st.code(metin, language="text")
        st.caption("Yukarıdaki metni seçip kopyalayarak WhatsApp veya e‑postada paylaşabilirsiniz.")
    else:
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("📜 Sertifika Uyarıları"):
            uyarilar = sertifika_uyarilari_al()
            if uyarilar:
                for u in uyarilar:
                    st.warning(f"{u['ad']} {u['soyad']} - {u['sertifika_adi']} ({u['makine']}) → {u['gecerlilik_tarihi']}")
            else:
                st.success("Yaklaşan sertifika bitişi yok.")

        with st.expander("📋 Bugünün Planı"):
            plan = bugun_plani_olustur()
            if plan:
                for p in plan:
                    durum = "✅" if "⚠️" not in p["Personel"] else "🟡"
                    st.write(f"{durum} {p['Gemi']} – {p['Makine']}: **{p['Personel']}**")
            else:
                st.info("Planlanmış görev yok.")

        st.markdown("---")
        if st.button("🌓 Tema Değiştir", key="tema_sidebar", help="Açık/koyu tema arasında geçiş yapar"):
            st.session_state.tema_koyu = not koyu
            st.rerun()
        st.markdown("---")
        st.caption("v5.8 · Buton yardımları eklendi")
