"""
Ordino Yağcı Planlaması — Tüm Butonlara caption eklendi (v5.8)
...
def _sayfa_yapboz():
    ...
    with c_btns:
        ...
        if cols_hafta[0].button("⬅️ Hafta"):
            st.caption("Bir önceki haftaya gider")
        ...
        if cols_hafta[1].button("Hafta ➡️"):
            st.caption("Bir sonraki haftaya gider")
            sec_tarih = st.session_state.yapboz_tarih + timedelta(days=7)
            st.session_state.yapboz_tarih = sec_tarih
            st.rerun()
    ...
    with col1:
        temizle_btn = st.button("🧹 Tüm Atamaları Temizle", key="temizle_yapboz", type="secondary")
        ...
        if temizle_btn:
            ...
            st.rerun()
    with col2:
        otodoldur_btn = st.button("🤖 Hepsini Otomatik Doldur", key="otomatik_doldur_btn")
        ...
        for i, mak in enumerate(g_makineler):
            with cols[i]:
                ...
                if mevcut:
                    ...
                    with col_x:
                        cikar_btn = st.button("❌ Çıkar", key=f"c_{gemi['id']}_{mak['id']}_{sec_tarih}")
                        ...
                        if cikar_btn:
                            ...
                            st.rerun()
                    with col_d:
                        degistir_btn = st.button("🔄 Değiştir", key=f"degistir_{gemi['id']}_{mak['id']}_{sec_tarih}")
                    ...
                    onerbtn = st.button("🔍 Öneri Al (5)", key=f"onerbtn_{gemi['id']}_{mak['id']}_{sec_tarih}")
                    ...
                    if f"oneriler_{gemi['id']}_{mak['id']}" in st.session_state and st.session_state[f"oneriler_{gemi['id']}_{mak['id']}"]:
                        ...
                        for o in oneriler:
                            ...
                            with col_o1:
                                ...
                                st.write(puan_str)
                            with col_o2:
                                ata_btn = st.button("✅ Ata", key=f"ata_{gemi['id']}_{mak['id']}_{o['id']}")
...
def _sayfa_excel():
    ...
    with st.form("f_gemi"):
        ...
        if makineler:
            ...
            sec_mak = []
        submit_gemi = st.form_submit_button("➕ Gemi Ekle")
    ...
    with st.expander("➕ Makine Tipi Ekle"):
        with st.form("f_makine"):
            ...
            submit_makine = st.form_submit_button("➕ Makine Ekle")
    ...
    with st.expander("🔗 Gemi-Makine Eşleştirme (Düzenle)"):
        ...
        if sec_gemi:
            ...
            guncelle_btn = st.button("Güncelle", key="gm_guncelle")
            st.caption("Seçili makineleri gemiyle eşleştirir")
            if guncelle_btn:
    ...
    with c1:
        with st.expander("✏️ Gemi Düzenle/Sil"):
            if g_rows:
                ...
                guncelle_btn = st.button("Güncelle",key="bgd")
                st.caption("Geminin adını, kodunu veya konumunu günceller")
                if guncelle_btn:
                ...
                sil_btn = st.button("Sil",key="bgs",type="secondary")
                st.caption("Gemiyi tüm bağlantılarıyla siler")
                if sil_btn:
    ...
    with c2:
        with st.expander("✏️ Makine Düzenle/Sil"):
            ...
            if mr:
                ...
                guncelle_btn = st.button("Güncelle",key="bmd")
                st.caption("Makine adını günceller")
                if guncelle_btn:
                ...
                sil_btn = st.button("Sil",key="bms",type="secondary")
                st.caption("Makineyi siler (bağlı personel varsa izin vermez)")
                if sil_btn:
...
def _sayfa_personel():
    ...
    with st.expander("🔄 Toplu Durum Değiştir"):
        ...
        toplu_durum_btn = st.button("Durumları Güncelle", key="toplu_durum_btn")
        st.caption("Seçili personelin durumunu topluca günceller")
        if toplu_durum_btn:
            if sec_personel:
    ...
    with st.expander("📤 Excel'den Toplu Personel Ekle"):
        ...
        if personel_excel is not None:
            try:
                ...
                ekle_btn = st.button("📤 Ekle", key="btn_toplu_personel")
                st.caption("Excel'deki tüm satırları personel olarak kaydeder")
                if ekle_btn:
                    eklenen = 0
    ...
    with st.expander("➕ Yeni Personel"):
        ...
        kaydet_btn = st.button("Kaydet",key="btn_pk")
        st.caption("Yeni personeli sisteme ekler")
        if kaydet_btn:
    ...
    with st.expander("✏️ Düzenle/Sil & Sertifika"):
        ...
        c1,c2=st.columns(2)
        guncelle_btn = c1.button("Güncelle",key="bpgu")
        ...
        sil_btn = c2.button("Sil",key="bps",type="secondary")
        ...
        with st.form("sert_ekle",clear_on_submit=True):
            ...
            sert_ekle_btn = st.form_submit_button("Ekle")
        ...
        if sert:
            ...
            sert_sil_btn = st.button("Sertifika Sil",key="bss")
            st.caption("Seçili sertifikayı siler")
            if sert_sil_btn:
...
def _sayfa_izin():
    ...
    with st.expander("📤 Excel'den Toplu İzin Ekle"):
        ...
        if izin_excel is not None:
            try:
                ...
                st.dataframe(df.head(), use_container_width=True)
                ekle_btn = st.button("📤 Ekle", key="btn_toplu_izin")
                st.caption("Excel'deki izinleri topluca sisteme aktarır")
                if ekle_btn:
                    eklenen = 0
                    for _, row in df.iterrows():
                        ad = str(row.get('Ad', '')).strip().upper()
    ...
    with cf:
        ...
        kaydet_btn = st.button("✅ Kaydet",key="biz")
        st.caption("İzni kaydeder. Görev çakışırsa uyarı gösterir")
        if kaydet_btn:
    ...
    if not izinler: st.info("İzin yok")
        ...
        for iz in izinler:
            ...
            sil_btn = c3.button("🗑️",key=f"izsil_{iz['id']}")
...
def _sayfa_carkci():
    ...
    olustur_btn = st.button("Oluştur",key="bck")
    st.caption("Çarkçı kaydını kaydeder ve varsa personel puanını düşürür")
    if olustur_btn:
        if not ad or not soyad: st.error("Ad soyad zorunlu")
        else:
...
def _sayfa_acil():
    ...
    with st.expander("📅 Dün Kim Çalıştı?"):
        ...
        sorgu_btn = st.button("🔍 Sorgula", key="dun_sorgu")
        st.caption("Dünkü pozisyonun sahibini gösterir")
        if sorgu_btn:
    ...
    with st.expander("🏝️ Personeli İskeleye Çıkar"):
        ...
        iskele_btn = st.button("🔄 İskeleye Çıkar", key="btn_iskele")
        st.caption("Personeli 'İskelede' yapar")
        if iskele_btn:
    ...
    with st.expander("🚀 İskeledekileri Akıllı Dağıt"):
        akilli_btn = st.button("🧠 Akıllı Dağıtım Başlat", key="btn_akilli_dagit")
        st.caption("İskelede/İZİNCİ personeli boş yerlere otomatik yerleştirir")
        if akilli_btn:
    ...
    bos_btn = st.button("🔍 Listele",key="bbos")
    st.caption("Bugün hiç görevi olmayan personeli listeler")
    if bos_btn:
        bos=[]
        for p in sql_all("SELECT * FROM personel WHERE aktif=1 AND (vardiya_tipi='IZINCI' OR durum IN ('Gemide','İskelede'))"):
    ...
    iskele_btn2 = st.button("🔍 İskele Listesi",key="biskele")
    st.caption("'İskelede' durumundaki personeli gösterir")
    if iskele_btn2:
    ...
    tersane_btn = st.button("🔍 Tersane Listesi",key="btersane")
    st.caption("Tersanedeki gemilere uygun personeli listeler")
    if tersane_btn:
    ...
    oner_btn = st.button("🚨 Öner (5)",key="bacil")
    st.caption("İzin isteyenin yerine en uygun 5 adayı getirir")
    if oner_btn:
...
def _sayfa_oneri():
    ...
    with st.expander("Ayarlar"):
        ...
        olustur_btn = st.button("🚀 Oluştur",key="btp")
        st.caption("Seçili aralıktaki tüm boş pozisyonları adilce doldurur")
        if olustur_btn:
    ...
    with st.expander("🗑️ Vardiya Sil"):
        ...
        if mevcut_sil:
            ...
            sil_ata_btn = st.button("Atamayı Sil",key="sil_ata")
            st.caption("Belirtilen atamayı kaldırır")
            if sil_ata_btn:
    ...
    oner_btn = st.button("🔍 Öner",key="bon")
    st.caption("Seçili pozisyon için en uygun 5 adayı listeler")
    if oner_btn:
    ...
    kaydet_btn = st.button("✅ Kaydet",key="bvk")
    st.caption("Seçili personeli elle atar")
    if kaydet_btn:
...
def _sayfa_bilgi():
    ...
    c1,c2,c3=st.columns(3)
    with c1:
        yedek_btn = st.button("💾 Yedekle",key="byedek")
        st.caption("Veritabanının yedeğini alır")
        if yedek_btn:
            st.toast(f"Yedek: {veritabani_yedekle().name}", icon="💾")
    with c2:
        test_btn = st.button("🧪 Test Verisi",key="btest")
        st.caption("Örnek gemi, makine, personel ve izin kayıtları oluşturur")
        if test_btn:
            test_verisi_olustur()
    with c3:
        st.download_button("📥 DB İndir",open(DB_PATH,"rb"),file_name=f"ordino_{date.today().isoformat()}.db",
                           help="Veritabanı dosyasını bilgisayara indirir")
        st.caption("Veritabanı dosyasını bilgisayara indirir")
    st.divider()
    st.subheader("📄 PDF")
    cp1,cp2=st.columns(2)
    with cp1:
        pdfa_btn = st.button("Aylık Özet PDF",key="bpdfa")
        st.caption("Aylık personel özeti PDF'i oluşturur")
        if pdfa_btn:
    ...
    with cp2:
        ...
        pdf_btn = st.button("PDF Oluştur", key="pdf_aralik")
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        tema_btn = st.button("🌓 Tema Değiştir", key="tema_sidebar")
        st.caption("Açık/koyu tema arasında geçiş yapar")
        if tema_btn:
