...
Hatalar giderildi, tüm modüller dosyaya entegre edildi.
"""
...
# ---------------------------------------------------------------------------
# YAPILANDIRMA & SABİTLER (config, constants)
# ---------------------------------------------------------------------------
DEFAULT_AYARLAR = {
...
# ---------------------------------------------------------------------------
# TARİH YARDIMCILARI (vardiya_kurallari)
# ---------------------------------------------------------------------------
GUNLER_TR = ["Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi", "Pazar"]

def gun_sayisi(bas: date, bit: date) -> int:
    """İki tarih arasındaki gün sayısı (dâhil)."""
    return (bit - bas).days + 1

def izin_pzt_3gun(bas: date) -> tuple[date, date]:
    """Pazartesi başlayan izni Pzt–Sal–Çar olarak 3 güne ayarlar."""
...
# ---------------------------------------------------------------------------
class YagciOneri:
    def __init__(self, sira: int, personel_id: int, ad_soyad: str, gemi: str, makine: str,
                 vardiya_tipi: str, is_kalitesi: int, performans_notu: str | None, uyari: str = ""):
        self.sira = sira
        self.personel_id = personel_id
        self.ad_soyad = ad_soyad
        self.gemi = gemi
        self.makine = makine
        self.vardiya_tipi = vardiya_tipi
        self.is_kalitesi = is_kalitesi
        self.performans_notu = performans_notu or ""
        self.uyari_8_5 = uyari
        # Basit skor: iş kalitesi puanı
        self.skor = is_kalitesi

def to_dict_rows(oneri_list: list[YagciOneri]) -> list[dict]:
    return [vars(o) for o in oneri_list]

def onerileri_hesapla(gemi_id: int, makine_id: int, hedef_tarih: date,
                      cikan_yagci_id: int | None = None, limit: int = 5) -> list[YagciOneri]:
    """
    Gerçek öneri motoru:
    - Aktif, gemiden çekilmemiş, çarkçı sorunu olmayan personeli al.
    - Aynı gemide çalışmıyor olmalı (gemi_id'den farklı).
    - Aynı makine tipine sahip olmalı (makine_id).
    - Çıkan yağcı varsa onu hariç tut.
    - İş kalitesine göre sırala, ilk 'limit' kadarını döndür.
    """
    query = """
        SELECT p.id, p.ad, p.soyad, g.ad AS gemi_ad, m.ad AS makine_ad,
               p.vardiya_tipi, p.is_kalitesi, p.performans_notu
        FROM personel p
        LEFT JOIN gemi g ON g.id = p.gemi_id
        LEFT JOIN makine_tipi m ON m.id = p.makine_tipi_id
        WHERE p.aktif = 1
          AND p.gemiden_cekilme = 0
          AND p.carkci_ile_sorun = 0
          AND p.makine_tipi_id = ?
          AND p.gemi_id != ?
    """
    params = [makine_id, gemi_id]
    if cikan_yagci_id is not None:
        query += " AND p.id != ?"
        params.append(cikan_yagci_id)

    query += " ORDER BY p.is_kalitesi DESC LIMIT ?"
    params.append(limit)

    rows = sql_all(query, tuple(params))
    oneriler = []
    for idx, r in enumerate(rows, 1):
        uyari = ""
        if r["vardiya_tipi"] == "8_5":
            uyari = "8/5 çalışanı, dikkat"
        o = YagciOneri(
            sira=idx,
            personel_id=r["id"],
            ...
            performans_notu=r["performans_notu"],
            uyari=uyari,
        )
        oneriler.append(o)
    return oneriler
...
def bugun_plani_olustur() -> list[dict]:
    """Bugünün vardiya planını döndürür (henüz tam entegre edilmedi)."""
    # Gerçek planlama verisi olmadığı için boş dönüyoruz.
    # İleride burada vardiya_plan tablosu sorgulanabilir.
    return []
...
def render_yapboz() -> None:
    ...
    st.info("Bu modül şu anda geliştirilme aşamasındadır. Çok yakında sürükle-bırak atama arayüzü eklenecek.")
    st.markdown("Burada personeli gemilere sürükleyerek vardiya planı oluşturabileceksiniz.")
...
def _sayfa_personel() -> None:
    ...
    if not gemiler or not makineler:
        st.warning("Önce Gemiler sekmesinden en az bir gemi ve makine tipi ekleyin.")
        return
        ...
        if st.button("Personel kaydet", key="btn_personel_kaydet"):
            if not ad or not soyad:
                ...
                sql_run("""
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?,?)
                """, (ad, soyad, int(gid), int(mid), vt, gun_json, gemi_tutumu, izin_gun_json,
                      f"{izin_bas.strftime('%H:%M')} - {izin_bit.strftime('%H:%M')}", int(is_kalitesi),
    ...
    with st.expander("Personel düzenle / sil"):
        ...
        if c1.button("Personel bilgisini güncelle", key="btn_personel_guncelle"):
            sql_run("""
                UPDATE personel SET gemiden_cekilme = ?, carkci_ile_sorun = ?, carkci_sorun_notu = ? WHERE id = ?
            """, (1 if gemiden_cekildi == "Evet" else 0, 1 if carkci_sorun == "Evet" else 0,
                  sorun_notu.strip() if carkci_sorun == "Evet" and sorun_notu.strip() else None, int(pid)))
...
def _sayfa_carkci() -> None:
    ...
    if st.button("Çarkçı kaydı oluştur ve yağcıyı öneri dışı bırak", key="btn_carkci_kaydet"):
        sql_run("""INSERT INTO carkci(ad, soyad, gemi_id, problemli_yagci_id, sorun_metni, vardiya_notu, carkci_vardiya)
                   VALUES (?,?,?,?,?,?,?)""", (ad, soyad, int(gid), int(yid), sorun, vn, carkci_vardiya))
...
def _sayfa_bilgi() -> None:
    ...
    aktif_izinde = int((sql_one("SELECT COUNT(*) AS c FROM izin WHERE date('now') BETWEEN baslangic AND bitis") or {"c": 0})["c"])
...
- Tersane (8/5) personeli: **{tersane}**
    ...
    gemi_bazli = sql_all("""
        ...
        FROM gemi g LEFT JOIN personel p ON p.gemi_id = g.id GROUP BY g.id, g.ad ORDER BY g.ad
    """)
    ...
    if plan_bugun:
        ...
        st.caption("Bugün için vardiya planı henüz oluşturulmadı. Yapboz sekmesinden atama yapabilirsiniz.")
...
def main() -> None:
    ...
    [data-testid="stAppViewContainer"] .main .block-container {
      background: rgba(255, 255, 255, 0.96); border-radius: 14px; padding: 1rem 1rem 1.2rem;
      border: 1px solid #ffd2a1; box-shadow: 0 12px 34px rgba(28, 17, 8, 0.22);
    }
    h1, h2, h3, h4, p, li, label, span, div { color: #2f251b !important; }
    [data-testid="stForm"] { background: #fffaf4; border: 1px solid #ffd8b0; border-radius: 12px; padding: 0.9rem 1rem 0.4rem; }
    [data-testid="stExpander"] { border: 1px solid #ffd8b0; border-radius: 12px; background: #fffdf9; }
    .stButton button { width: 100%; min-height: 44px; background: #f3831f; color: #ffffff; border: 1px solid #d66d12; border-radius: 10px; font-weight: 600; }
    .stButton button:hover { background: #d96f14; border-color: #bf5f10; transform: translateY(-1px); }
    .stDataFrame, .stTable { background: #ffffff; border-radius: 10px; border: 1px solid #f7d7b4; }
    </style>
    ...
    if not st.session_state.get("ordino_auth"):
        ...
        return
    _logout()
    st.sidebar.caption("Şifreyi değiştirmek için `ORDINO_ADMIN_USER` / `ORDINO_ADMIN_PASSWORD` ortam değişkenlerini kullanın.")
    with st.sidebar.expander("Planlama ayarları", expanded=False):
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["Gemiler", "Personel", "İzin", "Çarkçı", "Öneri", "Yapboz (v8)", "Bilgi"])
    with tab1: _sayfa_gemiler()
    with tab2: _sayfa_personel()
    with tab3: _sayfa_izin()
    with tab4: _sayfa_carkci()
    with tab5: _sayfa_oneri()
    with tab6: render_yapboz()
    with tab7: _sayfa_bilgi()
